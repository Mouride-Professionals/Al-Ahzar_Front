...
const { pages: { dashboard, stats: { amount } }, components: { menu } } = messages;
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            const {
                alazhar: {
                    get: {
                        finance: { all: paymentsRoute, statsWithoutSchoolId: paymentStatsRoute },
                        expenses: { all: expensesRoute, statsWithoutSchoolId: expenseStatsRoute },
                    },
                },
            } = routes.api_route;

            try {
                const paymentStatsUri = selectedSchool === "all"
                ...
                const expenseStatsUri = selectedSchool === "all"
                ...
                const paymentStatsData = await serverFetch({ uri: paymentStatsUri, user_token: token });
                const expenseStatsData = await serverFetch({ uri: expenseStatsUri, user_token: token });
                ...
                toast({ title: "Erreur", description: "Échec du chargement des données", status: "error", duration: 5000, isClosable: true });
            } finally {
                setLoading(false);
            }
        };
            ...
            <Stack spacing={5}>
                ...
                <Tabs variant="soft-rounded" colorScheme="orange" onChange={(index) => setActiveTab(index === 0 ? "payments" : "expenses")}>
                    ...
                    <TabPanels>
                        ...
                        <TabPanel>
                            <Wrap spacing={20.01}>
                                <HStack w="100%"><Statistics cardStats={paymentStats} /></HStack>
                        ...
                        <TabPanel>
                            <Wrap spacing={20.01}>
                                <HStack w="100%"><Statistics cardStats={expenseStats} /></HStack>
...
export const getServerSideProps = async ({ req, res }) => {
    ...
    const {
        alazhar: {
            get: {
                ...
                finance: { all: payments, statsWithoutSchoolId: financeStats },
                expenses: { all: expenses, statsWithoutSchoolId: expenseStats },
            },
        },
    } = routes.api_route;
    ...
    const [schoolsData, initialPaymentKpis, initialExpenseKpis] = await Promise.all([
        serverFetch({ uri: schoolsRoute, user_token: token }),
        serverFetch({ uri: financeStats.replace("%activeSchoolYear", activeSchoolYear), user_token: token }),
    ...
    return {
        props: {
            ...
            initialPaymentKpis,
            initialExpenseKpis,
            schools: schoolsData.data.map((s) => ({ id: s.id, name: s.attributes.name })),
            activeSchoolYear,
        },
    };
};
