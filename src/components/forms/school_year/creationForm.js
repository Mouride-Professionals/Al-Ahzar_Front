...
// Separate component for form content to properly handle hooks
const SchoolYearFormContent = ({
  values,
  errors,
  touched,
  handleChange,
  ...
  }, [values.startDate, values.endDate, values.name, isEdit, setFieldValue]);

  const {
    inputs: {
      schoolYear: {
        creation: { startDate, endDate, description },
      },
    },
    messages: {
      schoolYear: {
...
export const CreateSchoolYearForm = ({
  ...
  return (
    ...
    >
      {(formikProps) => (
        <SchoolYearFormContent {...formikProps} isEdit={isEdit} t={t} />
      )}
    </Formik>
  );
};
