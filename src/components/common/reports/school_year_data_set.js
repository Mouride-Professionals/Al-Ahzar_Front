            ...
            <SimpleGrid columns={[1, null, 2]} spacing={5} pt={3}>
              <VStack align="start" spacing={3}>
                ...
                <HStack>
                  ...
                  <Text
                    color={
                      isCurrent ? 'green.500' : isEnded ? 'red.500' : 'gray.500'
                    }
                  >
                    {isCurrent
                      ? t('current')
...
export const SchoolYearDataSet = ({ role, data = [], columns, token }) => {
  const t = useTranslations('components.dataset.schoolYears');
  ...
  const actionButton = hasPermission(role.name, 'manageSchoolYears') && (
    <Button
      onClick={() =>
  ...
  return (
    <DataTableLayout
      ...
      expandedComponent={(data) => ExpandedComponent({ ...data, role, token })}
